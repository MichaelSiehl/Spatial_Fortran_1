...
**To still satisfy the requirement of segment ordering with coarray accesses, we must now apply parallel programming styles and models (with our parallel logic codes else) to ensure the now required ‘*sequentially consistent memory ordering*’ (see DPC++ and section 8.1 here) with it.**
