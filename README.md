...
Coarray Fortran does inherently allow asynchronous execution of coroutines (see section 8.3). **Only the required non-blocking synchronization method is not explicitly implemented with the language because it would certainly break with the coarray requirement of *execution segment ordering***. Instead, we can implement a user-defined non-blocking synchronization method using (atomic) coarrays. This is very much the same as with a blocking synchronization, but with the exception that it does not contain a blocking spin-wait loop with a receive. Simply said, we remove this blocking spin-wait loop away from the synchronization and place it else as encompassing parallel loop for the execution control of the (asynchronous) coroutines.<br />
**To still satisfy the requirement of *execution segment ordering* with coarray accesses, we must now apply parallel programming styles and models (with our parallel logic codes else) to ensure the now required ‘*sequentially consistent memory ordering*’ (see DPC++ and section 8.1 here) with it.**
...
Additionaly, I am using an integer-based enumeration technique to assign values to atomic operations in the coroutines. These assigned integer-based enum values are another key to ensure the required ***sequentially consistent* memory ordering** with the execution of the coroutines here, **because atomic operations are ordered yet**. (As stated for DPC++, this must be supported by a specific device).



### 8.2  Fault Tolerant Execution with Pairwise Independent Forward Progress

The execution sequence of the kernels in a coroutine is sequentially ordered through the channel’s *i_chstat* argument, even if the ordering may change during execution. A crucial property is a constant swap of channel send and receive for successive execution of kernels on the coarray images. This is to ensure pairwise (independent) forward progress for parallel kernel execution.
...
As the channel’s *IsReceive* function is non-blocking (see code example 6-2), we can use the encompassing parallel loop to implement an execution control that simultaneously executes multiple such MODULE PROCEDURES inside that parallel loop. A spatial compiler should be enabled to generate multiple (or merge into) spatial pipelines from the control flow (if-else / select-case) on FPGAs.
...
The last kernel in *code example 8-2* does call the channel’s *IsReceive* function on the single-task (*control*) image. That call does already collect data from the multiple sends of the multi-task (*execute*) images. That is also why we have to use arrays here, for the receive of the multiple scalar values.
